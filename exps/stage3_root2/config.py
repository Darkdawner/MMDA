...
class Config:
    ...
    OUTPUT_DIR = osp.join(ROOT_DIR, 'model_logs_2', osp.split(osp.split(osp.realpath(__file__))[0])[1])
    ...
    MODEL = edict()
    MODEL.STAGE_NUM = 2
    MODEL.UPSAMPLE_CHANNEL_NUM = 128

    MODEL.DEVICE = 'cuda'

    MODEL.WEIGHT = osp.join(ROOT_DIR, 'lib/models/resnet-50.pth')

    # -------- Training Config -------- #
    SOLVER = edict()
    SOLVER.IMG_PER_GPU = 10
    SOLVER.BASE_LR = 3.8e-4
    SOLVER.CHECKPOINT_PERIOD = 1000
    SOLVER.MAX_ITER = 46000
