...
from dataset.ImageAugmentation import (aug_croppad, aug_flip, aug_rotate, aug_scale, AID_img, Noise_img)
from dataset.representation import generate_heatmap, generate_paf, generate_yz_heatmap_aug, generate_yz_heatmap, generate_high_yz_heatmap, generate_rel_yz_heatmap, generate_xz_heatmap_aug
...
class JointDataset(Dataset):
    def __init__(self, cfg, stage, transform=None, with_augmentation=False, with_mds=False):
        ...
        if self.stage == 'train':
            ...
            with open(DATASET.MPII_JSON_PATH) as data_file:
        ...
        self.root_path = {}
        for dname in (['COCO', 'MPII'] + DATASET.USED_3D_DATASETS): # 'MUCO', 'CMUP', 'H36M'
        ...
        self.params_transform['square_size'] =  DATASET.TRAIN.SQUARE_SIZE
    ...
    def __getitem__(self, index):
        ...
        if data['dataset']=='COCO':
            img = cv2.imread(osp.join('/fsa/home/dataset/COCO2017/', data['img_paths']).replace('\\','/'), cv2.IMREAD_COLOR)
            if img is None:
                data = copy.deepcopy(self.train_data[0])
        ...
        img = cv2.imread(osp.join(root_path, data['img_paths']).replace('\\','/'), cv2.IMREAD_COLOR)
        # pair_path = data['img_paths'].replace('\\','/').split('/')
        # img_pair = cv2.imread(osp.join(root_path, pair_path[0], 'unaugmented_set', pair_path[2], pair_path[3]), cv2.IMREAD_COLOR)
        ...
        # generate labels
        valid = np.ones((self.keypoint_num*3 + self.paf_num*2, 1), np.float)
        if meta_data['dataset'] == 'COCO':
        ...
        if meta_data['dataset'] == 'MPII':

            valid[self.keypoint_num + self.paf_num*2:, 0] = 0
        
        # high_output_shape = (self.output_shape[0]*4, self.output_shape[1]*4)
        xz_output_shape = (512,208)
        yz_output_shape = (128,512)

        labels_num = len(self.gaussian_kernels)
        labels = np.zeros((labels_num, self.keypoint_num + self.paf_num*2, *self.output_shape))
        xz_labels = np.zeros((labels_num, self.keypoint_num, *xz_output_shape))
        yz_labels = np.zeros((labels_num, self.keypoint_num, *yz_output_shape))
        for i in range(labels_num):
            ...
            # pafs
            labels[i][self.keypoint_num:] = generate_paf(meta_data['bodys'], self.output_shape, self.params_transform, self.stride,\
                                                         self.paf_num, self.paf_vector, max(1, (3-i))*self.paf_thre, self.with_mds)

            yz_labels[i] = generate_yz_heatmap_aug(meta_data['bodys'], meta_data['scale'], yz_output_shape, self.stride, \
                                                             self.keypoint_num, kernel=self.gaussian_kernels[i])
            xz_labels[i] = generate_xz_heatmap_aug(meta_data['bodys'], meta_data['scale'], xz_output_shape, self.stride, \
                                                             self.keypoint_num, kernel=self.gaussian_kernels[i])

    
        labels = torch.from_numpy(labels).float()
        yz_labels = torch.from_numpy(yz_labels).float()
        xz_labels = torch.from_numpy(xz_labels).float()
        valid = torch.from_numpy(valid).float()

        return img, valid, labels, xz_labels, yz_labels
