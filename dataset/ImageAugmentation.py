...
def Noise_img(img):
    ...
    if dice < 0.5:
        ...
        mask_prob = np.random.rand(h, w)
        thre = 1-random.random()*0.7
        mask_prob[mask_prob<thre] = 0
        mask_prob[mask_prob>=thre] = 1
        for i in range(c):
            mask[:,:,i] *= mask_prob
...
def AID_img(img, square_size):

    dice = random.random()

    if dice < 0.5:
        h,w,_ = img.shape
        h_num = int(h/square_size)
        w_num = int(w/square_size)

        mask = np.random.rand(h_num, w_num)
        mask[mask<0.5]=0
        mask[mask>=0.5]=1
